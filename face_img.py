...
def init_dict():
  ...
  for i,a in enumerate(os.listdir(test_data_dir)):
    print("\rInitialization Number - {} - {}\t\t\t\t".format(i,a),end="")
...
if __name__=="__main__":
  ...
  def stream():
    ...
    global threshold,device,mtcnn,resnet,inmem_embeddings_dict,cap,vote_dict,i,font,fontScale,color,thickness,tcolor
    ...
    try:
      ...
      try:
        for box in boxes:
            ...
            frame = cv2.putText(frame, str(name), (int(box[0]),int(box[1])), font,  
      ...
      cv2.imwrite('fr.jpg',cv2.cvtColor(frame,cv2.COLOR_BGR2RGB))
      exit(0)
